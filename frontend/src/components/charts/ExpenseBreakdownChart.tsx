import React from 'react';
import { useTheme } from '../../context/ThemeContext';
import { MOCK_EXPENSE_CATEGORIES } from '../../mock/dashboardData';

export const ExpenseBreakdownChart: React.FC = () => {
  const { tokens: t } = useTheme();
  const categories = MOCK_EXPENSE_CATEGORIES;
  const total = categories.reduce((a, c) => a + c.amount, 0);

  return (
    <div
      style={{
        background: t.card,
        border: `1px solid ${t.border}`,
        borderRadius: 28,
        padding: 22,
      }}
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: t.text, fontFamily: "'Manrope', sans-serif" }}>
            Expense breakdown
          </h3>
          <p style={{ margin: '3px 0 0', fontSize: 12.5, color: t.textSub, fontFamily: "'Inter', sans-serif" }}>
            Operational expenditure distribution
          </p>
        </div>
        <span style={{ fontSize: 13, fontWeight: 700, color: t.text, fontFamily: "'Manrope', sans-serif" }}>
          Total: ₹{total.toFixed(1)}Cr
        </span>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {categories.map((c, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, fontFamily: "'Inter', sans-serif" }}>
              <span style={{ color: t.text, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: i === 0 ? t.accent : i === 1 ? t.ok : t.warn }} />
                {c.category}
              </span>
              <span style={{ color: t.textSub, fontWeight: 600 }}>
                ₹{c.amount}Cr ({c.percentage}%)
              </span>
            </div>
            <div style={{ width: '100%', height: 6, borderRadius: 999, background: t.track, overflow: 'hidden' }}>
              <div
                style={{
                  width: `${c.percentage}%`,
                  height: '100%',
                  borderRadius: 999,
                  background: i === 0 ? t.accent : i === 1 ? t.ok : t.warn,
                  transition: 'width 0.4s ease',
                }}
              />
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};
