"""
BizPilot AI - Cashflow Risk & Forecasting Model Training & Selection Script
"""
...
MODELS_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../models/cashflow"))
os.makedirs(MODELS_DIR, exist_ok=True)
DATASET_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "../datasets/cashflow_forecasting.csv"))


def load_and_preprocess_cashflow_data():
    if not os.path.exists(DATASET_PATH):
        raise FileNotFoundError(f"Cashflow Forecasting dataset missing at {DATASET_PATH}")

    df = pd.read_csv(DATASET_PATH)
    
    # Drop rows where target is NaN (last row)
    df = df.dropna(subset=['target_future_closing_cash_next_month']).reset_index(drop=True)
...
def evaluate_cashflow_model(model, X_eval, y_eval, min_cash_threshold: float = 40000000.0):
    ...
    # Business Cash Risk Period Validation (Low Liquidity < Threshold)
    actual_risk = (y_eval < min_cash_threshold).astype(int)
...
def train_and_select_cashflow_model():
    print("=" * 80)
    print("      PROBLEM 3: CASHFLOW FORECASTING & RISK ANALYSIS")
    print("=" * 80)

    (X_train, X_val, X_test, y_train, y_val, y_test, 
     feature_cols, X_train_val, y_train_val, 
     train_dates, val_dates, test_dates) = load_and_preprocess_cashflow_data()
    ...
    for name, model in candidates.items():
        ...
        metrics = evaluate_cashflow_model(model, X_val, y_val)
        results[name] = {
            "model": model,
            "metrics": metrics
        }
    ...
    final_test_metrics = evaluate_cashflow_model(final_model, X_test, y_test)
    print("\n===================================================================")
    ...
    print(f"  Risk Detection:            Correct={final_test_metrics['risk_detection']['correct_detected']}, Missed={final_test_metrics['risk_detection']['missed']}, FalseAlarm={final_test_metrics['risk_detection']['false_alarm']}")
    print("===================================================================")

    # Serialize Model & Save Metadata
    model_path = os.path.join(MODELS_DIR, "cashflow_model_v1.pkl")
    meta_path = os.path.join(MODELS_DIR, "metadata.json")

    joblib.dump(final_model, model_path)

    metadata = {
        "model_name": "cashflow_forecasting",
        "version": "1.0",
        "algorithm": best_name,
        "features": feature_cols,
        "target": "target_future_closing_cash_next_month",
        "training_date": datetime.now().isoformat(),
        "dataset_version": "cashflow_forecasting_v1",
...
if __name__ == "__main__":
    train_and_select_cashflow_model()
