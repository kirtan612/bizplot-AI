"""
BizPilot AI - Financial Profit Forecasting Explainability & Drivers Module
"""
...
def explain_profit_predictions() -> dict:
    """
    Computes global feature importances and historical business drivers for Profit Forecasting.
    """
    if not os.path.exists(MODEL_PATH) or not os.path.exists(META_PATH):
        raise FileNotFoundError("Profit model artifact or metadata missing.")

    model = joblib.load(MODEL_PATH)
    with open(META_PATH, "r", encoding="utf-8") as f:
        meta = json.load(f)

    df = pd.read_csv(DATASET_PATH).dropna(subset=['target_future_profit_next_month'])
    ...
    print("--- FINANCIAL PROFIT MODEL-IMPORTANT DRIVERS ---")
